 ...
 * @package LobbyChatwing\SDK\Api
 */

namespace LobbyChatwing\Api;
...
class Action extends Object
{
     ...
     * @param array $params
     *
     * @throws \LobbyChatwing\Exception\ChatwingException
     */
    ...
    {
        if (is_null($path)) {
            if (!defined('LOBBY_CHATWING_BASE_DIR')) {
                define('LOBBY_CHATWING_BASE_DIR', dirname(dirname(__FILE__)));
            }
            $path = dirname(LOBBY_CHATWING_BASE_DIR) . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'actions.php';
        }

        if (file_exists($path)) {
            self::$actionList = include $path;
        } else {
            throw new ChatwingException(array('message' => __("Action list not found", LOBBY_CHATWING_TEXTDOMAIN), 'code' => 0));
        }
    }
